...
| `make-prebuilt-apps.sh` | Regenerates the `prebuilt-apps/` bundles (works on any OS). |
| `prebuilt-apps/` | Ready-made `Privacy Cleanup.app` + `Privacy Cleanup Preview.app`. |
...
**Prebuilt (in this repo):** `prebuilt-apps/` contains ready-made bundles —
`Privacy Cleanup Preview.app` (dry run) and `Privacy Cleanup.app` (asks y/N,
then deletes). Each embeds its own copy of `privacy-cleanup.sh`, so you can
drag just the `.app` to `/Applications`. They're generated by
`make-prebuilt-apps.sh` (re-run it after editing the main script). Because
they're unsigned and downloaded, macOS quarantines them — clear it once:
```bash
xattr -dr com.apple.quarantine "/path/to/prebuilt-apps"
```
(or double-click once, then System Settings → Privacy & Security → **Open
Anyway**). After that they launch like any app. On double-click the app opens
a Terminal window via `open -a Terminal` — no AppleScript, so there's no
"wants to control Terminal" Automation prompt. Note: downloading individual
files through the GitHub web UI strips the executable bit; clone the repo or
use a zip that preserves permissions.

**Build locally (avoids quarantine entirely):**
```bash
./build-app.sh            # builds "Privacy Cleanup.app" (runs the preview)
./build-app.sh --apply    # builds "Privacy Cleanup (Apply).app"
```
Then drag the `.app` to `/Applications` or your Dock. This AppleScript route
will ask to let the app control Terminal
